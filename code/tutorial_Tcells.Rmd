...
data_Tcell %>%
    ...
    # ?Todo: What is happening? ? Todo: What is geom_col doing?
...
We now compute the UMAPs for each T cell compartment across all samples (acute and convalescent). We use all markers, except the parameters we used for gating. We also exclude IgD, IgM, CD21, and CD14.  
...
```{r computeUMAP}
transformed_scaled_data_Tcell <- data_Tcell %>%
...
UMAP_TCRgd <- transformed_scaled_data_Tcell %>%
...
UMAP_CD4 <- transformed_scaled_data_Tcell %>%
...
UMAP_CD8 <- transformed_scaled_data_Tcell %>%
...
p1 <- data_Tcell %>%
    # Apply scale() on all columns defined by sel_markers_Tcells
    mutate_at(vars(sel_markers_Tcells), scale) %>%
    # Keep Tcellcompartment CD4+ cells
    filter(Tcellcompartment == "CD4+") %>%
...
p2 <- data_Tcell %>%
    # Apply scale() on all columns defined by sel_markers_Tcells
    mutate_at(vars(sel_markers_Tcells), scale) %>%
    # Keep Tcellcompartment CD8+ cells
    filter(Tcellcompartment == "CD8+") %>%
    # Subsample 30000 cells
    sample_n(30000) %>%
    # Plot the two just calculated UMAPs, color according to HLADR values
...
p3 <- data_Tcell %>%
    # Apply scale() on all columns defined by sel_markers_Tcells
    mutate_at(vars(sel_markers_Tcells), scale) %>%
    # Keep Tcellcompartment TCRgd+ cells
    filter(Tcellcompartment == "TCRgd+") %>%
    # Plot the two just calculated UMAPs, color according to HLADR values
    ggplot(aes(x = UMAP1, y = UMAP2, color = HLADR)) +
    # Make points
...
data_Tcell %>%
    # Apply scale() on all columns defined by sel_markers_Tcells
    mutate_at(vars(sel_markers_Tcells), scale) %>%
    # Keep cells in Tcellcompartment CD8+ and with acute Disease.phase
