...
We start reading the data table "data_norm_sub.csv" with the function `read.csv()`. The data was already pre-processed 
    1. Remove dead cells ???HOW
    2. Remove doublets ???HOW
    3. Remove debris ???HOW
    4. and batch-corrected ???HOW
...
data_norm_sub %>% 
    ...
    # ?Todo what?
    pivot_longer(names_to = "marker",values_to = "value",panel) %>%
    # mutate() generates a new column "log_value" and takes the logaritm (log())
    # of the column "value" but adds 1 before. 
    # We add 1 because some "value"-s can be 0 and the logarithm is not defined here
    mutate(asinh_value = asinh(value)) %>% 
    # Start plotting, define "log_value" as x-axis
    ggplot(aes(x=asinh_value)) +
...
???Why geom_point and not geom_hex()
...
data_norm_sub %>% 
 # ???Why are we using only positive values here?
 # filter retains only rows where the rules inside apply, 
 # so each row where CD45>0 and CD3>0 stays, all other rows are removed
 filter(CD45>0, CD3>0) %>% 
 # Use only 10% of the data 
 sample_frac(0.1) %>% 
 # Before, we used mutate() to generate a single new column. 
 # mutate_at() changes all values (here vars(panel)) and applies the following 
 # function (here asinh()) 
...
data_norm_sub <- data_norm_sub %>% 
    ...
    # ifelse returns the value inside if TRUE when the logical value is TRUE, otherwise the other value.

    # ???Why sinh? replace.  
    mutate(Tcell = ifelse(CD3>sinh(4.3) & CD45>sinh(1) & CD15<sinh(4) & CD19<sinh(2.9), TRUE, FALSE))
```
...
# facet_grid splits the plots according to multiple values, you could even use multiple splitting-values
...
We are actually interested in finding T cell communities but in each main T cell compartment (CD4+, CD8+ and TCRgd+). For this, we manually gate T cells based on CD8 and TCRgd parameters.  
...
# Then the dataframe "gates" is joined with data_Tcell using ???WHICH COLUMN AS JOIN?
 left_join(gates) %>% 
...
data_Tcell %>% 
# Transform all panel columns with asinh
  mutate_at(vars(panel),asinh) %>% 
...
data_Tcell %>% 
    # Count how many values there are in every Tcellcompartment
    count(Tcellcompartment) %>% 
    # ?Todo: What is happening with that mutate, also before?
    mutate(perc = n/sum(n)*100) %>% 
...
We now compute the UMAPs for each T cell compartment across all samples (acute and convalescent). We use all markers, except the parameters we used for gating. We also exclude IgD, IgM, CD21, and CD14.  ??? Why do we exclude the gating-parameters and the other parameters?

We define a vector "sel_markers_Tcells" removing all parameters we do not want to include for the calculation of the UMAP.

```{r sel_markers_Tcells}
# panel %in% c("text1", "text2", ...) 
#   Returns a vector which is TRUE if its corresponding value in "panel" is in the vector on the right
...
```{r computeUMAP}
transformed_data <- data_Tcell %>%
    # Apply asinh() on all columns defined by sel_markers_Tcells
...
We now plot each UMAP, coloured by severity, disease phase, and intensity of a selected marker. 
...
p1 <- data_Tcell %>% 
    ...
    sample_n(30000) %>% 
    # Plot the two just calculated UMAPs, color according to severity
...
??? Do we have answers to all of these things?
