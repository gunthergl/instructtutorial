...
data_norm_sub %>% 
 ...
 geom_point(size = 0.01, alpha = 0.1) +
 geom_density_2d()+
 geom_rect(mapping=aes(xmin=4, xmax=8, ymin=4.3, ymax=8), color="black", alpha=0) 
...
* Add a column to the data table where each cell gets the classification T cell = {TRUE, FALSE} according to your gating strategy. (flagging)
...
```{r columnTcells}
data_norm_sub <- data_norm_sub %>% mutate(Tcell = ifelse(CD3>sinh(4.3) & CD45>sinh(4) & CD15<sinh(4) & CD19<sinh(2.9), TRUE, FALSE))
```
